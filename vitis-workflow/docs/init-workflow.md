...
`sim.lobster` is now available as the first post-init workflow and follows the same pattern: batch Tcl execution, result-file handoff, structured state capture, and a final report-only AI review. `synth.lobster` now follows the same structure for `csynth_design`, including report extraction for timing/resource summaries. See `docs/sim-workflow.md` and `docs/synth-workflow.md`.
